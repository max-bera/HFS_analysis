function F = double_power_law_objfun(x, oscfreq, Ecomp)
...
%fit function
%%
storagemod = real(x(1).*(1i*oscfreq).^x(2)+x(3).*(1i*oscfreq).^(x(4))-Ecomp);

lossmod = imag(x(1).*(1i*oscfreq).^x(2)+x(3).*(1i*oscfreq).^(x(4))-Ecomp);
